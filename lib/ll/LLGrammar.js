    ...
    for (let i = 0; i < productions.length; i++) {
      ...
      for (const r of rhs) {
        ...
        if (!parents) {
          ...
          parentNodeClassMap.set(toAstNodeClassName(rhName), parents);
        }
        parents.add(clsName);
      }
    }
    ...
    for (const p of productions) {
      ...
      for (const r of rhs) {
          ...
          if (p.skipAstNode) {
            ...
            while (true) {
              ...
              if (prev.length !== parents.length) {
                const filtered = prev.filter((p) => !f(p));
                if (filtered.every((f) => !!processed[f])) {
                  break;
                }
                for (const ff of filtered) {
                  processed[ff] = 1;
                  if (parentNodeClassMap.get(ff)) {
                    parents = parents.concat(
                      Array.from(parentNodeClassMap.get(ff)),
                    );
                  }
                }
              } else {
        ...
        if (skipNodeClass[cls]) {
          ...
          let seed = { [cls]: 1 };
          while (true) {
            const currentNodesLength = Object.keys(nonSkipNode).length;
            ...
            if (
              currentNodesLength === Object.keys(nonSkipNode).length &&
              seedLength === Object.keys(seed).length
            ) {
              break;
            }
          }
          delete nonSkipNode[cls];

          const arrayClass = Object.keys(nonSkipNode);
          rhsTypes.push(...arrayClass);
          looseRhType = true;
        } else {
          rhsTypes.push(cls);
        }
