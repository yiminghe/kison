...
var { toClassName, serializeObject, assertSymbolString, filterRhs } = Utils;
...
class LLGrammar extends Grammar {
  ...
  genDTs(base) {
    ...
    for (let i = 0; i < productions.length; i++) {
      ...
      let clsName = toClassName(p.symbol) + '_Node';
    ...
    for (const p of productions) {
      ...
      for (const r of rhs) {
        ...
        const name = toClassName(rhName);
        const cls = name + '_Node';
    ...
    for (const symbol of symbolMap.keys()) {
      ...
      if (classes.length) {
        const className = toClassName(symbol) + '_Node';
    ...
    for (const tokenName of tokenMap.keys()) {
      const className = toClassName(tokenName) + '_Node';
      allClassNames.push(className);
      ...
      if (col) {
        for (const terminal of Object.keys(col)) {
          ...
          if (ps !== undefined) {
            ...
            ret.push(
              (ps > 0 ? '' : '-: ') +
              `${nonTerminal} ${terminal} => ${production.symbol} -> ${filterRhs(production.rhs).join(', ') || 'EMPTY'
              }`,
            );
          }
        }
