...
const { IF } = require('../examples/vba/grammar/es/names');
...
class Grammar {
  ...
  toBNF() {
    ...
    function getSymbolBnf(r) {
      ...
      let quantifier = isOptionalSymbol(r)
        ? '?'
        : isZeroOrMoreSymbol(r)
        ? '*'
        : '';
      if (lexer.hasToken(s)) {
        line.push(`'${s}'`);
  ...
  expandProductionAlternation() {
    if (this.__expandProductionAlterations) {
      return;
    }
