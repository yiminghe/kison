...
Lexer.prototype = {
  ...
  genCode(cfg) {
    ...
    const isCompress = compressState || compressSymbol;

    var newCfg = serializeObject(
      { rules: self.rules, isCompress: isCompress },
      isCompress
        ? function(v) {
            var ret;
            if (v && v.regexp) {
  ...
  getCurrentRules() {
    ...
    const isCompress = self.isCompress;
    each(self.rules, function(r) {
      var filter = isCompress ? r[4] : r.filter;
      if (filter) {
        if (filter.call(self)) {
          rules.push(r);
        }
      ...
      var state = isCompress ? r[3] : r.state;
      if (!state) {
