...
Generate time: Sat Oct 09 2021 19:32:39 GMT+0800 (中国标准时间)
*/
var formulaParser = (function (undefined) {
  ...
  var parser = {
    productions: [
      ...
      ['arguments', ['exp?', 'argumentPart*']],
      ['argumentPart', ['ARGUMENT_SEPARATOR', 'exp?']],
      ...
      ['table-this-row', ['TABLE_@']],
      ['table-this-row', ['TABLE_@', 'TABLE_COLUMN_SPECIFIER']],
      ...
      [
        ...
        ['table-specifier-item', 'table-specifier-item-part*'],
      ],
      [
  ...
  const productionSkipAstNodeSet = new Set([
    18, 22, 34, 35, 37, 38, 39, 40, 41, 42, 44, 46, 47, 49, 50, 52, 59, 61, 63,
  ...
  const predictProductionIndexLLK = function (
  ...
  const predictProductionIndexNextLLK = function (arg) {
    return predictProductionIndexLLK(arg, () => lexer.lex());
  };
  const bfsMatch = function (startState) {
  ...
  const getNextReachableStates = function (reachableStates, ret) {
    const stack = [];
  ...
  const buildRhsSM = function (symbol, rhs, ruleIndex) {
    function getUnit(rr) {
      if (isSymbol(rr)) {
  ...
  const SymbolState = class SymbolState {
  ...
  const SymbolStateUnit = class SymbolStateUnit {
  ...
  const concatUnits = function (type, us, ruleIndex) {
    if (!us.length) {
      return null;
    }
    const l = us.length;
  ...
  const RootSymbolUnit = class RootSymbolUnit {
    units = undefined;
    start = undefined;
    end = undefined;
    type = 'rootSymbol';
