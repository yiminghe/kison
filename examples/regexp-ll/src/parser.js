...
Generate time: Sat Jan 08 2022 00:07:13 GMT+0800 (中国标准时间)
*/
var $parser = (function (undefined) {
  ...
  var my = {
    ...
    matchOnlyEscapeChar: function (lexer, index = 0) {
      let m = '';
      let char2 = lexer.nextChar(index);
      m += char2;
      if (char2 === '\\') {
        char2 = lexer.nextChar(index + 1);
        m += char2;
      } else {
        return { m, r: false };
      }
          ...
          if (lexer.options.unicode && index === 0) {
            if (first >= 55296 && first <= 56319) {
              ...
              if (second >= 56320 && second <= 57343) {
                let totoal = (first - 55296) * 1024 + second - 56320 + 65536;
                return {
                  m,
                  r: [
    ...
    matchAnyChar: function (lexer) {
      const char2 = lexer.nextChar();
      if (char2 === '.' && !lexer.userData.insideCharacterGroup) {
        return [char2];
      }
      return false;
    },
    ...
    matchGroupName: function (lexer, prefix) {
      ...
      let char2 = lexer.nextChar(index++);
      while ((char2 >= 'a' && char2 <= 'z') || (char2 >= 'A' && char2 <= 'Z')) {
        ret.push(char2);
        char2 = lexer.nextChar(index++);
      }
      if (char2 === '>') {
        const name = ret.join('');
    ...
    matchNumber: function (lexer, start = 0, hex, max) {
      ...
      while (index < l) {
        const char2 = lexer.nextCharAt(start + index).toLowerCase();
        if (char2 < '0' || char2 > '9') {
          if (hex) {
            if (char2 < 'a' || char2 > 'f') {
              break;
