...
class Grammar {
  ...
  toBNF() {
    const terminals = {};
    var { rules } = this.lexer;
    terminals[Lexer.STATIC.EOF_TOKEN] = 1;
    each(rules, rule => {
      var token = this.lexer.getRuleItem(rule, "token");
      if (token) {
        terminals[token] = 1;
      }
    });
  ...
  buildTerminals() {
    ...
    each(rules, rule => {
      var token = lexer.mapSymbol(lexer.getRuleItem(rule, "token"));
      if (token) {
        terminals[token] = 1;
      }
    });
